...
const AvailabilityCalendar = ({ professionalId }) => {
  ...
  useEffect(() => {
    const fetchAvailability = async () => {
      try {
        const response = await fetch(`http://localhost:3002/api/availability/${professionalId}?date=${selectedDate}`);
  ...
  const handleCreateSlot = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch('http://localhost:3002/api/availability', {
        method: 'POST',
        headers: { 
